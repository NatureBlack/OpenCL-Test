...
int main() {
  ...
  // make program
  string cl_path(CL_PATH);
  cl_path.append("/PI.cl");
  string source = clLoadProgramSource(cl_path.c_str());
  // string source(PI_SourceCode);
  vector<pair<const char*, ::size_t>> ps(
      1, make_pair(source.c_str(), source.size()));
  ...
  if (err != CL_SUCCESS) {
    string errString;
    checkCLError(program.getBuildInfo(vd[0], CL_PROGRAM_BUILD_LOG, &errString));
    printf("%s\n", errString.c_str());
    exit(1);
  }
